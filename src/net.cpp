...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "xyy " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"45.32.166.49", "node.45.32.166.49"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. xyy is probably already running."), addrBind.ToString().c_str());
        else
