...
#if defined(NDEBUG)
# error "xyy cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xad1ca8db18400b0940cc68dcc6f0204436333a4a24704f772391057f10901195");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // xyy: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "xyy Signed Message:\n";
...
{
    // xyy: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 1 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 3919995 * COIN;
            }

    // Subsidy is cut in half every 839999 blocks
    nSubsidy >>= (nHeight / 839999);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // xyy: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // xyy: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1476204008, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "fgdgf";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041365c9fa6fc884933c0880a382d3c10b1813183956d7819b39b27494216daeb2f4b10909954b7150249b46cdbdefd21a2be4301105eac0d713eee0a9055a6983") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1476204008;
        ...
        assert(block.hashMerkleRoot == uint256("0x43b3d3a8402381cc03a278bfdd283a2366b144bbe9cc1216abf01acb0782db4e"));
...
unsigned char pchMessageStart[4] = { 0xf4, 0x67, 0x8c, 0x61 }; // xyy: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("xyyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("xyyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static xyyMiner(CWallet *pwallet)
{
    printf("xyyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xyy-miner");
    ...
    try { loop {
        ...
        printf("Running xyyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&xyyMiner, pwallet));
}
